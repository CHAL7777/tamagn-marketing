...
import { HuggingFaceAssistant } from "@/components/ai/HuggingFaceAssistant";
import { getCurrentLocale } from "@/lib/i18n/server";
import { getDictionary } from "@/lib/i18n/translations";
import { cn } from "@/lib/utils";

const trustPillarIcons = [BadgeCheck, HandCoins, Truck] as const;
const roleCards = [
  { icon: ShoppingBag, href: "/products" },
  { icon: Store, href: "/choose-role" },
  { icon: BriefcaseBusiness, href: "/services" },
] as const;
  ...
  return (
    <main className="page-shell pb-24 pt-8 md:pt-10">
      <section className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div className="space-y-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              {dictionary.home.bookServices}
            </Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="metric-card bg-surface-container-low p-5">
              <p className="section-kicker">{dictionary.home.metrics.trustModel}</p>
              <p className="mt-2 text-xl font-extrabold text-foreground">
                {dictionary.home.metrics.trustModelValue}
              </p>
            </div>
            <div className="metric-card bg-surface-container-low p-5">
              <p className="section-kicker">{dictionary.home.metrics.payments}</p>
              <p className="mt-2 text-xl font-extrabold text-foreground">
                {dictionary.home.metrics.paymentsValue}
              </p>
            </div>
            <div className="metric-card bg-surface-container-low p-5">
              <p className="section-kicker">{dictionary.home.metrics.operations}</p>
              <p className="mt-2 text-xl font-extrabold text-foreground">
                {dictionary.home.metrics.operationsValue}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="editorial-card relative overflow-hidden p-8 md:p-10">
          ...
          <div className="relative space-y-8">
            <div className="glass-panel rounded-[1.75rem] p-6">
              <p className="section-kicker">{dictionary.home.platformPromiseKicker}</p>
              <h2 className="mt-3 text-3xl font-black tracking-[-0.05em]">
                {dictionary.home.platformPromiseTitle}
              </h2>
              <p className="mt-3 text-sm leading-7 text-secondary">
                {dictionary.home.platformPromiseBody}
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.75rem] bg-primary p-6 text-on-primary shadow-[0_24px_48px_rgba(1,110,0,0.18)]">
                ...
                <p className="mt-4 text-sm font-bold uppercase tracking-[0.18em] text-primary-fixed">
                  {dictionary.home.platformPromiseKicker}
                </p>
                <p className="mt-2 text-2xl font-black tracking-[-0.04em]">
                  {dictionary.home.hyperlocalDiscovery}
                </p>
              </div>
              <div className="rounded-[1.75rem] bg-surface-container-low p-6">
                ...
                <p className="mt-4 text-sm font-bold uppercase tracking-[0.18em] text-secondary">
                  {dictionary.home.metrics.payments}
                </p>
                <p className="mt-2 text-2xl font-black tracking-[-0.04em]">
                  {dictionary.home.buyerProtection}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="mt-16 grid gap-5 md:grid-cols-3">
        {dictionary.home.trustPillars.map(({ title, body }, index) => {
          const Icon = trustPillarIcons[index] ?? BadgeCheck;
          return (
      ...
      <section className="mt-20 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="section-shell">
          <p className="section-kicker">{dictionary.home.howItWorksKicker}</p>
          <h2 className="mt-3 text-4xl font-black tracking-[-0.05em]">
            {dictionary.home.howItWorksTitle}
          </h2>
      ...
      <section className="mt-20 grid gap-5 lg:grid-cols-3">
        {dictionary.home.roles.map(({ title, body, cta }, index) => {
          const role = roleCards[index] ?? roleCards[0];
          ...
          <article key={title} className="editorial-card p-7">
            ...
            <p className="mt-3 text-sm leading-7 text-secondary">{body}</p>
            <Link href={role.href} className="eyebrow-link mt-6">
              {cta}
              <ArrowRight className="size-4" />
            </Link>
          </article>
      ...
      <section className="mt-20">
        <HuggingFaceAssistant locale={locale} labels={dictionary.ai} />
      </section>
    </main>
  );
}
